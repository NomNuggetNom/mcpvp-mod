...
public class GuiEditHUD extends GuiScreen {
	...
	public void drawScreen(int p_73863_1_, int p_73863_2_, float p_73863_3_) {
		Draw.centeredString(Format.s("gui.edit.stop"), 0, this.height/4, this.width, 0xFFFFFF, true);
		
		if (Selectable.selected != null) {
			Selectable current = Selectable.selected;
			Draw.centeredString(Format.s("gui.edit.selected") + current.getName() + "#r#\" (" + current.getX() + ", " + current.getY() + ")",
					0, this.height/4 + 15, this.width, 0xFFFFFF, true);
			
			if (current instanceof InfoBlock || current instanceof InfoBox) {
				Draw.centeredString(Format.s("gui.edit.edit"),
